<?php
/**
 * Elkorrekt AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Elkorrekt AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through el_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'elkorrekt', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'elkorrekt' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'elkorrekt' ) ) );

		register_block_pattern_category( 'elkorrekt-sections', array( 'label' => __( 'Elkorrekt AB sections', 'elkorrekt' ) ) );
		register_block_pattern_category( 'elkorrekt-pages', array( 'label' => __( 'Elkorrekt AB page layouts', 'elkorrekt' ) ) );
	}
);
