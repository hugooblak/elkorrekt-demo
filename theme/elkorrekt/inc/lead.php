 ...
 * or photo. They call el_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Elkorrekt AB.
 *
 ...
 * @return string
 */
function el_sample_note() {
	return el_lead( 'ar_riktig' ) ? '' : ' <span class="el-sample">(platshållare)</span>';
}
 ...
 * @return string File name for el_lead_img().
 */
function el_lead_bild( $i ) {
	$bilder = (array) el_lead( 'bilder', array() );
 ...
 * @return string
 */
function el_ring_label() {
	$namn = el_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, el_lead( 'telefon' ) ) : 'Ring ' . el_lead( 'telefon' );
}
