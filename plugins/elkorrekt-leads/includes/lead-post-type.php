...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'elkorrekt-leads' ),
					'singular_name' => __( 'Lead', 'elkorrekt-leads' ),
					'edit_item'     => __( 'Förfrågan', 'elkorrekt-leads' ),
					'search_items'  => __( 'Search leads', 'elkorrekt-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'elkorrekt-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'elkorrekt-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'elkorrekt-leads' ),
			'npl_phone'    => __( 'Phone', 'elkorrekt-leads' ),
			'npl_service'  => __( 'Needs', 'elkorrekt-leads' ),
			'npl_timeline' => __( 'When', 'elkorrekt-leads' ),
			'npl_zip'      => __( 'ZIP', 'elkorrekt-leads' ),
			'npl_source'   => __( 'Source', 'elkorrekt-leads' ),
			'npl_status'   => __( 'Status', 'elkorrekt-leads' ),
			'date'         => __( 'Received', 'elkorrekt-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'elkorrekt-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'elkorrekt-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'elkorrekt-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'elkorrekt-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'elkorrekt-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'elkorrekt-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'elkorrekt-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'elkorrekt-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'elkorrekt-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'elkorrekt-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'elkorrekt-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'elkorrekt-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
