...
add_filter(
	...
	function ( $exporters ) {
		$exporters['elkorrekt-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'elkorrekt-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'elkorrekt-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'elkorrekt-leads',
						'group_label' => __( 'Quote requests', 'elkorrekt-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['elkorrekt-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'elkorrekt-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
